...
function emptyForm() {
  return {
    ...
    class: '', weeklyFee: getDefaultWeeklyFee(),
    notes: '',
  };
}
...
export default function EnrollmentForm({ onClose, onSaved }) {
  ...
  function submit() {
    if (!validate()) return;
    const s = addStudent({ ...form, weeklyFee: Number(form.weeklyFee) });
    setSaved(s);
    setDone(true);
    onSaved && onSaved(s);
  }
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="modal-footer">
          ...
          {!done && step === 2 && <button className="btn btn-gold" onClick={submit}>Enroll student</button>}
