        ...
        {students.filter(s=>s.class===activeClass).map(s=>{
          ...
          return (
            <div key={s.id} className="card" style={{cursor:'pointer',borderLeft:'3px solid var(--border-strong)',transition:'box-shadow 0.15s'}}
              onClick={()=>onSelect(s)}
              onMouseEnter={e=>e.currentTarget.style.boxShadow='0 4px 16px rgba(42,42,42,0.10)'}
              ...
              <div style={{display:'flex',gap:8,fontSize:12}}>
                ...
                <div style={{flex:1,background:'var(--amber-light)',borderRadius:6,padding:'6px 10px',textAlign:'center'}}>
              ...
              <div style={{marginTop:10,fontSize:12,color:'var(--text-muted)',display:'flex',justifyContent:'space-between'}}>
                ...
                <span style={{color:'var(--rose-dark)',fontWeight:500}}>View records →</span>
              </div>
            </div>
          );
        })}
...
function StudentRecords({ student, settings, onBack }) {
  ...
  async function summarise() {
    ...
    if (!monthDates.length) { setAiSummary('No records for this month yet. Add some daily entries first.'); return; }
    ...
    const prompt=`You are a helpful Madrasah assistant. Below are the daily records for ${student.forename} ${student.surname} at ${settings.schoolName} for ${new Date().toLocaleDateString('en-GB',{month:'long',year:'numeric'})}.\n\n${attLine}\n\n${entries}\n\nWrite a warm, professional monthly progress summary for this student's report. Cover: overall behaviour and attitude, key positives, any recurring concerns, and a brief recommendation. Around 150–200 words, paragraph form only.`;
    ...
    } catch { setAiSummary('Connection error. Please check your internet connection and try again.'); }
    setAiLoading(false);
  }

  const DayEntry = ({date, isToday}) => {
    const entry=getEntry(date);
    const isOpen=expanded[date];
    const hasContent=entry.comment||entry.positive||entry.negative;
    return (
      <div className="card mb-4" style={{borderLeft:`3px solid ${isToday?'var(--rose)':'var(--border-strong)'}`}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',marginBottom:isOpen?12:0}}
          ...
          <div className="flex items-center gap-2">
            {isToday&&<span className="badge badge-rose">Today</span>}
            <button className="btn btn-icon btn-sm" style={{color:'var(--red)'}}
              title="Delete this record"
              onClick={e=>{e.stopPropagation();setConfirmDel(date);}}>
              <Trash2 size={13}/>
            </button>
    ...
    <div>
      ...
      <div className="grid-2" style={{alignItems:'flex-start'}}>
        <div>
          ...
          {/* Today if not in list */}
          {!hasToday&&(
            <div className="card mb-4" style={{borderLeft:'3px solid var(--rose)'}}>
              <div className="flex justify-between items-center" style={{marginBottom:10}}>
                <div style={{fontWeight:600,fontSize:13}}>Today — {fmtDate(isoToday())}</div>
                <span className="badge badge-rose">Today</span>
              </div>
              <div className="form-group" style={{marginBottom:10}}>
                <label>Daily comment</label>
        ...
        <div style={{position:'sticky',top:24}}>
          <div className="card">
            ...
            {aiLoading&&<div className="ai-summary-box"><div style={{color:'var(--text-muted)',fontStyle:'italic',fontSize:13}}>Reading through {student.forename}'s records…</div></div>}
            {aiSummary&&!aiLoading&&(
            ...
            {!aiSummary&&!aiLoading&&(
              <div style={{textAlign:'center',padding:'16px 0',color:'var(--text-muted)',fontSize:13}}>
                ...
                Add daily records then click above to generate a report summary for {student.forename}.
              </div>
            )}
          </div>
          {/* Month stats */}
          <div className="card" style={{marginTop:14}}>
            ...
            {(()=>{
              ...
              return [['Days recorded',md.length,undefined],['With comments',md.filter(d=>getEntry(d).comment).length,'var(--charcoal)'],['With positives',md.filter(d=>getEntry(d).positive).length,'var(--green)'],['With concerns',md.filter(d=>getEntry(d).negative).length,'var(--red)']].map(([l,v,col])=>(
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setConfirmDel(null)}>
          <div className="modal" style={{maxWidth:400}}>
            <div className="modal-body" style={{textAlign:'center',paddingTop:28}}>
              ...
              <div style={{color:'var(--text-muted)',fontSize:13}}>{fmtDate(confirmDel)}</div>
              <div style={{color:'var(--text-muted)',fontSize:12,marginTop:6}}>This cannot be undone.</div>
            </div>
