...
export default function Students() {
  ...
  return (
    <Layout title="Students" subtitle={`${students.length} enrolled students`}>
      {/* Class tabs */}
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(s)}>
                  ...
                  <td className="text-muted text-sm">{s.dob}</td>
                  <td>
                    <div style={{ fontWeight: 500, fontSize: 13 }}>{s.parent1Name}</div>
                    <div className="text-muted text-sm">{s.parent1Phone}</div>
                  </td>
                  <td className="text-muted text-sm">
                    {s.parent2Name ? <><div style={{ fontWeight: 500, fontSize: 13 }}>{s.parent2Name}</div><div>{s.parent2Phone}</div></> : '—'}
                  </td>
                  <td>£{s.weeklyFee}/wk</td>
                  <td><span className={`badge ${s.status === 'Active' ? 'badge-brown' : 'badge-gray'}`}>{s.status}</span></td>
                  <td>
                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                      <button className="btn btn-icon btn-sm" title="View" onClick={() => setSelected(s)}><Eye size={13} /></button>
                      <button className="btn btn-icon btn-sm" title="Edit" onClick={() => startEdit(s)}><Pencil size={13} /></button>
                      <button className="btn btn-icon btn-sm" title="Delete" style={{ color: 'var(--red)' }} onClick={() => setConfirmDelete(s)}><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setSelected(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="grid-2 mb-4">
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    return [['Present', c.present, 'var(--teal-dark)'], ['Late', c.late, 'var(--gold)'], ['Absent', c.absent, 'var(--red)'], ['Total days', c.total, undefined]].map(([l, v, col]) => (
                      <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
              ...
              <div className="form-section-title" style={{ marginBottom: 10 }}>Parent / guardian contacts</div>
              <div className="grid-2 mb-4">
              ...
              {selected.notes && (
                <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic' }}>
                  "{selected.notes}"
                </div>
              )}
            </div>
            ...
            <div className="modal-body">
              ...
              <div className="form-grid form-grid-2 mb-5">
                ...
                <div className="form-group">
                  ...
                  <input type="number" value={editForm.weeklyFee} onChange={e => setEditForm({ ...editForm, weeklyFee: e.target.value })} />
                </div>
              </div>

              <div className="form-group">
                <label>Notes</label>
                <textarea rows={2} value={editForm.notes || ''} onChange={e => setEditForm({ ...editForm, notes: e.target.value })} style={{ resize: 'vertical' }} />
              </div>
            </div>
            <div className="modal-footer">
